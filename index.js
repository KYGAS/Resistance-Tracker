module.exports = function Tracker(mod) {
	...
	mod.hook('S_NPC_RESISTANCE', 1, { filter: { fake: null } }, (event) => {
		...
		let magicalShred = originalDefense[event.gameId].magicalResistance - event.magicalResistance
		let physicalShred = originalDefense[event.gameId].physicalResistance - event.physicalResistance

		event.magicalResistance = originalDefense[event.gameId].magicalResistance * (1 - magicPercent) - magicalShred - magicalIgnore
		event.physicalResistance = originalDefense[event.gameId].physicalResistance * (1 - physicalPercent)  - physicalShred - physicalIgnore
